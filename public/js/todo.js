...
function loadEdit() {
    ...
    if (task != undefined) {
        ...
        $('#edit-duedate').val(task.getDate().toDateInputValue());
...
function deleteTask(id) {
    var data = {id: id};
    $.post('/deletedb', data, function(res) {
        list[res].delete();
        delete list[res];
    }, 'json');
}
...
function createTask(note, date, priority) {
    ...
    $.post('/adddb', data, function(res) {
        var task = new Task(res.note, new Date(res.date), res.priority, res.id);
        list[res.id] = task;
        filterTasks();
    }, 'json');
}
...
function updateTask(note, date, priority, done, id) {
    ...
    $.post('/updatedb', data, function(res) {
...
function getTaskList() {
	$.getJSON('/tasklistdb', function(res) {
		$.each(res, function(index, task) {
			var dueDate = new Date(task.DueDate);
            var newTask = new Task(task.Title, dueDate, task.Priority, task.Id);
            if (task.Completed == 1) {
                newTask.finished();
            }
            if (task.ParentToDo == null) {
